...
### [2026-08-12] Corrige corte de imagem no carrossel do Hero e adiciona efeito de "vazamento" sobre "Como Funciona"

**O que foi pedido:** (1) as imagens do carrossel do Hero estavam sendo cortadas (`object-cover` numa moldura de proporção fixa); (2) a moldura do carrossel deveria se estender visualmente por cima da seção seguinte, como uma camada sobreposta.

**Antes de implementar:** o pedido descrevia o efeito sobre a seção "Sobre Nós", mas a ordem real das seções é Hero → **Como Funciona** → Sobre Nós → Planos. Perguntado ao usuário se o efeito deveria ir na seção que de fato vem depois do Hero (Como Funciona) ou se "Sobre Nós" deveria ser reordenada pra logo após o Hero — usuário confirmou manter a ordem atual e aplicar sobre "Como Funciona".

**O que foi feito:**
- **`HeroCarousel.tsx`:** troca `object-cover` → `object-contain` nas 2 variantes (estática de 1 imagem e nas imagens absolutas do carrossel de 2+) — a imagem completa (screenshot de produto) agora sempre aparece inteira dentro da moldura, com uma pequena margem interna (cor de fundo da própria moldura) quando a proporção não preenche 100%, em vez de cortar topo/base/laterais.
- **`Landing.tsx` (Hero):** removido o `overflow-hidden` da `<section>` do Hero — necessário pra moldura conseguir "vazar" visualmente pra fora dela; seguro porque a ilustração de fundo já é contida via `inset-0` (sem risco de vazar ela também). Wrapper da moldura ganhou `relative z-20 lg:self-end lg:-mb-32` — o vazamento e o `z-20` só entram a partir de `lg` (onde o Hero vira layout lado-a-lado; abaixo disso o texto/imagem empilham e o efeito ficaria estranho colado no texto).
- **`Landing.tsx` ("Como Funciona"):** conteúdo (título/cards) ganhou `relative z-30` — sempre pinta por cima da moldura que vaza (z-20), nunca o contrário; padding-top ganhou `lg:pt-28` (era `py-16 sm:py-20` uniforme) pra dar respiro entre o título e a parte invasora da moldura.

**Decisões técnicas:**
- **`lg:self-end` foi essencial e não óbvio:** a primeira tentativa (só a margem negativa, sem `self-end`) não vazava nada — a linha do grid tem a altura do texto (mais alto que a imagem), e `items-center` do container simplesmente centralizava a imagem dentro dessa altura, sem nada "abaixo" pra margem negativa empurrar contra. Ancorando a moldura na base da linha (`align-self: end`), a margem negativa passa a de fato empurrar o fim da moldura pra além do limite da própria linha/seção.
- **`-mb-32` (128px) e não um valor menor:** o container do Hero tem `pb-16 sm:pb-24` (96px de padding inferior, ativo de `sm` pra cima, incluindo `lg`/`xl`); uma margem negativa igual ou menor que esse padding é só "absorvida" por ele e não vaza nada (foi exatamente o que aconteceu com uma primeira tentativa de `-mb-16`/`-mb-24` — a moldura parava certinha na borda da seção, sem ultrapassar). `128px - 96px de padding = 32px` de vazamento real, visualmente perceptível sem exagerar.
- **`z-30` no conteúdo de "Como Funciona" em vez de confiar só na ordem do DOM:** um elemento posicionado com `z-index` explícito (a moldura, `z-20`) pinta por cima de qualquer elemento subsequente com `z-index: auto` (o fundo da seção "Como Funciona", que não tem posicionamento/z-index próprio) independente da ordem no HTML — por isso o conteúdo da seção precisa do próprio `z-index` (30, maior que 20) pra garantir que nunca fique atrás da moldura, não pela ordem em que aparecem no markup.
- **Validado com `npx tsc --noEmit`, `npm run build` e teste visual real via Playwright**, inspecionando `getBoundingClientRect()` da moldura/seções pra confirmar matematicamente a sobreposição (não só "parece que vaza"): em 1440px a moldura vai até 32px abaixo do fim do Hero, dentro de "Como Funciona". Testado em 1440/1024 (vaza, `lg`+) e 768/390 (empilhado, sem vazamento, como esperado abaixo de `lg`), em light e dark, sem erros de console.

### [2026-08-12] Corrige header coberto (bug de z-index) e aumenta o carrossel do Hero (altura alinhada ao H1 + largura)

**O que foi pedido:** (1) bug crítico — o header sticky estava sendo coberto por "Como Funciona" ao rolar a página; (2) a moldura do carrossel do Hero deveria começar na mesma altura do topo do H1 (hoje começa mais abaixo) e ficar um pouco maior/mais larga, sem voltar a cortar imagem nem forçar scroll horizontal no mobile.

**Causa raiz do bug (1):** na rodada anterior, o header ficou em `z-30` e o conteúdo de "Como Funciona" (adicionado ali mesmo pra pintar por cima da moldura que vaza, também `z-30`) **empatou** com ele. Em z-index empatado, quem vem depois no DOM pinta por cima — e "Como Funciona" vem depois do header — cobrindo-o ao rolar. O carrossel (`z-20`) nunca foi o culpado.

**O que foi feito:**
- **Escala de z-index formalizada e documentada em `docs/architecture.md`** (nova seção "Escala de z-index"): header/`Sidebar` = `z-50` (chrome de navegação, nunca coberto), conteúdo que precisa ficar acima de uma camada decorativa adjacente = `z-20`, a própria camada decorativa = `z-10` — reaproveitando faixas que várias partes do app já usavam informalmente (dropdowns em `z-20`, `Sidebar`/backdrop em `z-40`/`z-50`, popover em `z-[60]`, modais em `z-[100]`+, `Toast` em `z-[9999]`), só formalizando o que faltava.
- **`Landing.tsx`:** header `z-30` → `z-50`; moldura do Hero `z-20` → `z-10`; conteúdo de "Como Funciona" `z-30` → `z-20` — restaura a ordem correta (`50 > 20 > 10`).
- **`Landing.tsx` (tamanho/posição do carrossel):** grid do Hero passou de `lg:grid-cols-2` (50/50) para `lg:grid-cols-[1fr_1.15fr]` (coluna da imagem um pouco mais larga, proporcional); wrapper da moldura trocou `lg:self-end` por **`lg:self-stretch`** e a largura mobile/tablet subiu de `max-w-md` pra `max-w-lg`.
- **`HeroCarousel.tsx`:** moldura (e a `<img>` da variante de 1 imagem) ganharam `lg:h-full` — herdam a altura esticada do wrapper em vez da proporção fixa; a variante de 2+ imagens troca `aspect-[4/3] sm:aspect-[16/10]` fixo por `lg:aspect-auto lg:h-full` a partir de `lg` (mobile/tablet continuam com a proporção fixa de antes, sem mudança). `object-contain` (da rodada anterior) mantido em ambas as variantes.

**Decisões técnicas:**
- **`lg:self-stretch`, não `lg:self-start`:** a primeira tentativa pra alinhar o topo foi `self-start` (ancora o topo da moldura no topo da linha do grid = topo do H1) — resolve o alinhamento, mas **anula o vazamento**: com o topo fixo, a margem negativa não tem mais nada "abaixo" pra empurrar (o fim da moldura passa a ser simplesmente `topo + altura própria`, sem tocar a margem). Com `self-stretch`, a altura passa a ser `altura-da-linha − margin-bottom`; como a margem é negativa, isso *aumenta* a altura calculada além da própria linha, esticando a moldura pra baixo a partir do mesmo topo (H1) — union dos dois requisitos (topo alinhado + vazamento) numa só propriedade.
- **Consequência aceita: a altura absoluta da moldura varia por breakpoint** (502px em 1440px, 578px em 1024px) porque ela agora deriva da altura do bloco de texto (que reflow de forma diferente em cada largura) — inerente a "alinhar com o topo do H1"; não é tratado como bug, é a implicação direta do que foi pedido.
- **Confirmado via `getBoundingClientRect()` (não só visual) em 1440/1024/768/390:** topo da moldura = topo do H1 em `lg`+ (1440 e 1024), vazamento de 32px mantido nos dois, `z-index` do header (50) sempre acima do carrossel (10) mesmo rolando a página até a moldura ficar visualmente sob o header, sem scroll horizontal em nenhum breakpoint testado (`document.documentElement.scrollWidth === window.innerWidth` nos 4). Abaixo de `lg` (768/390), moldura maior (`max-w-lg`) mas empilhada normalmente, sem vazamento — como já era.
- **Validado com `npx tsc --noEmit` e `npm run build`**, sem erros novos.

### [2026-08-12] Corrige preview gigante do card "Sobre Nós" no Painel de Governança

**O que foi pedido:** o card de preview da imagem "Sobre Nós" (aba "Landing Page" da Governança) estava renderizando a imagem em tamanho gigante, exigindo scroll — diferente dos cards pequenos e proporcionais do carrossel Hero acima dele. Só o layout do preview deveria mudar, nada de upload/storage/consumo na landing pública.

**Causa raiz (confirmada por repro isolado, não só inspeção visual):** `LANDING_IMAGE_SECTIONS` (`landingImages.ts`) tem hoje uma única entrada (`sobre_nos`) desde que o Hero saiu pra sua própria tabela e as 4 chaves `feature_*` foram removidas (ver entrada de 2026-08-12 mais acima, "Corrige uploads órfãos..."). A grade `gridTemplateColumns: repeat(auto-fit, minmax(280px, 1fr))` foi pensada pra até 6 cards; com um só, o `1fr` estica o único card pra ocupar 100% da largura do painel (quase 1200px numa tela comum) — e como o preview usava `w-full aspect-video` (altura = 9/16 da largura do próprio card), a altura ficava proporcional a essa largura gigante (~674px), não ao tamanho da imagem em si. `object-fit: cover` já estava certo — o "bug" nunca foi o corte da imagem, foi o card/moldura em si crescendo sem limite.

**O que foi feito (`LandingImagesPanel.tsx`):**
- `Card` de cada seção ganhou `max-w-sm` (384px) — antes esticava pra ocupar toda a largura da grade quando é o único item.
- Preview trocou `w-full aspect-video` por `w-full h-52` (altura fixa de 208px, dentro da faixa 200-250px pedida) — desacopla a altura da largura do card, então mesmo que a grade volte a ter mais colunas no futuro (nova seção 1:1 adicionada), a miniatura continua no tamanho certo.
- Botões de ação (trocar/remover) e todo o resto do card (upload, storage, `saveImageUrl`) **não foram tocados**.

**Decisões técnicas:**
- **Reproduzido antes de alterar o componente:** montado um HTML isolado com as classes exatas do card + a URL real da imagem já cadastrada (consultada direto no banco) + o CSS de build do próprio projeto, medindo `getBoundingClientRect()` via Playwright — confirmou os 1198×674px antes da correção (não só "parece grande"). Depois da correção, reproduzido de novo (agora injetando o HTML já corrigido numa página do dev server, pra usar o CSS gerado ao vivo pelo Tailwind) e confirmado 384×326px de card com preview de 208px de altura.
