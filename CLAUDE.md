...
O Claude Code deve ler, carregar e seguir rigorosamente as skills definidas na pasta `.claude/skills/`.

### Skills Ativas no Projeto:

1. **Clean Code (`.claude/skills/clean-code/SKILL.md`):** Regras de código limpo, SRP, nomes claros e escoteiro.
2. **Clean Architecture (`.claude/skills/clean-architecture/SKILL.md`):** Princípios SOLID e separação em camadas — aplicar de forma pragmática ao contexto React/Vite + Supabase do projeto (não há backend Nest/DDD aqui).
3. **Master Refactoring Specialist (`.claude/skills/Master-Refactoring-Specialist/SKILL.md`):** Catálogo de code smells e refatorações nomeadas (Fowler) para uso ao refatorar código existente.
4. **Auto-Documentação:** Registra alterações em `docs/changelog.md` (+ doc modular relevante) ao finalizar cada tarefa.
5. **Solução de Problemas & QA (`.claude/skills/error-ajustes-feature/SKILL.md`):** Executa diagnósticos (`npx tsc --noEmit`), corrige falhas de tipagem/sintaxe e valida a compilação.
