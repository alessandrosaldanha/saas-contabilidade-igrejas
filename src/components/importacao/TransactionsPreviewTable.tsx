        ...
        <div className="flex items-center gap-2">
          ...
          {transactions.length > 0 && (
            <button
              ...
              className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-md border border-neutral-300 dark:border-white/20 text-xs font-medium text-neutral-500 dark:text-neutral-400 hover:bg-status-error/10 hover:text-status-error hover:border-status-error"
            >
          ...
          <tbody>
            {transactions.map((row) => {
              ...
              return (
                <tr
                  key={row.id}
                  className={`border-t border-neutral-200 dark:border-white/10 ${lowConfidence ? "bg-status-warning/10" : ""}`}
                >
